...
def read_qc_rows() -> list[dict[str, str]]:
    with QC_ARTICLES.open("r", encoding="utf-8-sig", newline="") as handle:
...
def write_article_registry(rows: list[dict[str, str]]) -> None:
    fieldnames = [
        "batch_id",
        "corpus_name",
        "source_archive",
    ...
    with ARTICLE_REGISTRY.open("w", encoding="utf-8-sig", newline="") as handle:
...
def write_exclusion_log(rows: list[dict[str, str]]) -> None:
    ...
    with EXCLUSION_LOG.open("w", encoding="utf-8-sig", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
            writer.writerow(
...
def copy_files(rows: list[dict[str, str]]) -> None:
    ensure_clean_dirs()
    for row in rows:
        source = SOURCE_DIR / row["file_name"]
        target_dir = KEEP_DIR if row["keep_drop_review"] == "keep" else DROP_DIR if row["keep_drop_review"] == "drop" else KEEP_DIR
...
def main() -> None:
    ...
    resolved_rows = resolve_rows(read_qc_rows())
    write_article_registry(resolved_rows)
    write_exclusion_log(resolved_rows)
    copy_files(resolved_rows)
    write_summary(resolved_rows)
    print(f"Wrote {ARTICLE_REGISTRY.name}, {EXCLUSION_LOG.name}, and {CLEAN_SUMMARY.name}")
    print(f"Copied {sum(row['keep_drop_review'] == 'keep' for row in resolved_rows)} keep files to {KEEP_DIR}")
    print(f"Copied {sum(row['keep_drop_review'] == 'drop' for row in resolved_rows)} drop files to {DROP_DIR}")

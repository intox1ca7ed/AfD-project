...
def paths_for(dataset: str) -> dict[str, Path]:
    ...
    return {
        ...
        "articles_csv": dataset_qc_dir / "qc_articles.csv",
        "duplicates_csv": dataset_qc_dir / "duplicate_groups.csv",
        "summary_md": dataset_qc_dir / "qc_summary.md",
    }
...
def parse_article(dataset: str, path: Path) -> ArticleRecord:
...
def write_articles_csv(path: Path, records: list[ArticleRecord]) -> None:
    fieldnames = [
        "dataset",
        "file_name",
        "file_path",
...
def write_summary(path: Path, dataset: str, records: list[ArticleRecord], group_counts: dict[str, int], duplicate_rows: list[dict[str, str]]) -> None:
    total = len(records)
    ...
    lines = [
        f"# Chemnitz {dataset.capitalize()} QC Summary",
        "",
        "## Overall",
        "",
...
def main() -> None:
    ...
    records = [parse_article(dataset, path) for path in paths]
    duplicate_rows, group_counts = assign_group_labels(records)
    write_articles_csv(cfg["articles_csv"], records)
    write_duplicates_csv(cfg["duplicates_csv"], duplicate_rows)
    write_summary(cfg["summary_md"], dataset, records, group_counts, duplicate_rows)
    print(f"Processed {len(records)} files for {dataset}")
    print(f"Wrote {cfg['articles_csv'].name}, {cfg['duplicates_csv'].name}, {cfg['summary_md'].name}")

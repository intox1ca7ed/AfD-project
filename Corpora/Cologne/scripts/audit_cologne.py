...
CORPUS_DIR = COLOGNE_DIR / "raw_unarchive"
...
def parse_article(path: Path) -> ArticleRecord:
    ...
    if date_idx is not None and date_idx > 0:
        ...
        if title_candidates:
            ...
            source = preferred or title_candidates
            title = max(source, key=len)
    ...
    publication_lc = (publication or "").lower()
    ...
    if any(marker in section_lc for marker in opinion_markers) or any(
        marker in title_lc for marker in opinion_markers
    ):
        low_value_reasons.append("opinion_or_commentary")
    if any(marker in section_lc for marker in letter_markers) or any(
        marker in title_lc for marker in letter_markers
    ):
        low_value_reasons.append("reader_letter")
    if body_word_count and body_word_count < 120:
        low_value_reasons.append("very_short")
    if body_word_count and body_word_count < 220 and any(
        marker in section_lc
        for marker in ["lokales", "vermischtes", "service", "szene", "stadtteile"]
    ):
        low_value_reasons.append("short_regional_brief")
    if not body:
    ...
    elif "short_regional_brief" in low_value_reasons:
        keep_drop_flag = "review"
        keep_drop_reason = "short_regional_variant"
...
def assign_group_labels(records: list[ArticleRecord]) -> tuple[list[dict[str, str]], dict[str, int]]:
    ...
    for _, indexes in exact_groups.items():
        exact_counter += 1
        gid = f"exact_{exact_counter:03d}"
        for idx in indexes:
            records[idx].exact_duplicate_group = gid
        duplicate_rows.append(
            build_group_row(records, gid, "exact_duplicate", indexes)
        )
    ...
    for indexes in connected_components(near_edges, len(records)):
        ...
        for idx in indexes:
            if not records[idx].near_duplicate_group:
                records[idx].near_duplicate_group = gid
        duplicate_rows.append(
            build_group_row(records, gid, "near_duplicate", indexes)
        )
    ...
    for indexes in connected_components(variant_edges, len(records)):
        ...
        for idx in indexes:
            if not records[idx].repeated_variant_group:
                records[idx].repeated_variant_group = gid
        duplicate_rows.append(
            build_group_row(records, gid, "regional_variant", indexes)
        )

    for rec in records:
        reasons = [rec.keep_drop_reason]
...
def write_duplicate_csv(rows: list[dict[str, str]]) -> None:
...
def write_summary(records: list[ArticleRecord], group_counts: dict[str, int], duplicate_rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        ...
        "## Assessment",
        "",
        corpus_assessment(records, group_counts, malformed),
        "",
        "## Outlet Frequency",
        "",
        "| Publication | Count |",
    ...
    exact_rows = [row for row in duplicate_rows if row["group_type"] == "exact_duplicate"]
    near_rows = [row for row in duplicate_rows if row["group_type"] == "near_duplicate"]
    variant_rows = [row for row in duplicate_rows if row["group_type"] == "regional_variant"]

    lines.extend(render_group_section("Duplicate Groups", exact_rows))
    lines.extend(render_group_section("Suspicious Near-Duplicate Groups", near_rows))
    lines.extend(render_group_section("Repeated Regional Variants", variant_rows))

    if malformed:
        lines.extend(["## Empty Or Malformed Records", ""])
        for rec in malformed:
            lines.append(f"- {rec.file_name}: {rec.malformed_reason}")
        lines.append("")

    drop_recs = [rec for rec in records if rec.keep_drop_flag == "drop"]
    review_recs = [rec for rec in records if rec.keep_drop_flag == "review"]

    lines.extend(["## Recommended Drops", ""])
    for rec in drop_recs:
        lines.append(f"- {rec.file_name}: {rec.keep_drop_reason}")
    lines.append("")
...
def corpus_assessment(records: list[ArticleRecord], group_counts: dict[str, int], malformed: list[ArticleRecord]) -> str:
    drop_share = sum(rec.keep_drop_flag == "drop" for rec in records) / max(len(records), 1)
    if malformed:
        return "The corpus is usable, but not clean enough to treat as a final core event corpus without filtering. Exact duplicates, regional-edition variants, opinion/commentary items, and short peripheral briefs should be removed or reviewed before analysis."
    if group_counts["exact_groups"] or group_counts["near_groups"] or drop_share > 0.15:
        return "The corpus is broadly usable, but it needs conservative filtering before analysis. Use the keep/drop flags, collapse exact duplicates, and manually review near-duplicate and regional-variant groups."
    return "The corpus looks clean enough for core event analysis, with only limited review needed for flagged edge cases."


def render_group_section(title: str, rows: list[dict[str, str]]) -> list[str]:
    lines = [f"## {title}", ""]
    if not rows:
        lines.append("- None")
        lines.append("")
...
def main() -> None:
    ...
    write_duplicate_csv(duplicate_rows)
